  ...
  fetch("https://gamertocoder.garena.co.th/api/assets")
    ...
    .then((data) => {
      if (typeof data == "number") {
        ...
        Banner = data.banner; //3 Items
        Logo = data.logo; //3 Items
        Wallpaper = data.wallpaper; //14 Items
        //ignore Benny data this sh*t is a duplicate
        Characters = data.characters; //35 Items
        Benny = data.benny
        document.getElementById("blockManImg").src = Logo[0];
        document.getElementById("blockManImg2").src = Benny[0];
        document.getElementById("head").appendChild = '<link rel="icon" type="image/x-icon" href="' + Benny[0] + '">';
